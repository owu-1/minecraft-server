...
latest_ghrelease() { local user=$1;local repo=$2;local index=$3;echo "Downloading $repo...";echo --url $(curl -s "https://api.github.com/repos/$user/$repo/releases" | jq -r '.[0].assets['$index'].browser_download_url') | curl --progress-bar -Lo $repo.jar --config -;}

# Latest Modrinth
# Usage: latest_modrinth project
latest_modrinth() { local project=$1;echo "Downloading $project...";curl --progress-bar -Lo $project.jar $(curl -s "https://api.modrinth.com/v2/project/$project/version" --data-urlencode 'loaders=["$LOADER"]&game_versions=["$VERSION"]' | jq -r '.[0]'.files[0].url);}

# Latest Jenkins
# Usage: latest_jenkins api project filenameTest
latest_jenkins(){ local api=$1;local job=$2;local build=$3;local filename_test=$4;echo "Downloading $job...";local file=$(curl -s "$api/job/$job/lastSuccessfulBuild/api/json" | jq -r --arg filename_test $filename_test '.artifacts | .[] | select(.fileName|test($filename_test)).relativePath' | cut -d ' ' -f1);curl --progress-bar -Lo $file "$1/job/$2/lastSuccessfulBuild/artifact/$file";}
